...
// ── SWIMMING ANIMATION — drives the whole GLB model ──────────────────────────
function animateRabbit(elapsed) {
    if (!rabbitLoaded) return; // wait until model is ready
    ...
    // ── Forward lean — slight pitch like swimming stroke
    rabbit.rotation.x = -0.12 + Math.sin(elapsed * 0.7) * 0.04;

    // ── Subtle left-right sway
    rabbit.position.x = Math.sin(elapsed * 0.6) * 0.12;

    // ── Breathing scale pulse
    const breathe = 1 + Math.sin(elapsed * 1.8) * 0.018;
    rabbit.scale.set(breathe, breathe, breathe);
}
...
function animateRabbit(elapsed) {
    if (!rabbitLoaded) return;

    // Kill any lingering GSAP tweens on rabbit so we have full control
    gsap.killTweensOf(rabbit.position);
    gsap.killTweensOf(rabbit.scale);
    gsap.killTweensOf(rabbit.rotation);

    // ── Body bob — up/down in water
    rabbit.position.y = -1.0 + Math.sin(elapsed * 1.4) * 0.09;

    // ── Side to side gentle sway
    rabbit.position.x = Math.sin(elapsed * 0.6) * 0.12;

    // ── Body rock — tilt while swimming
    rabbit.rotation.z = Math.sin(elapsed * 1.4) * 0.06;

    // ── Forward lean — pitch like swimming stroke
    rabbit.rotation.x = -0.12 + Math.sin(elapsed * 0.7) * 0.04;
