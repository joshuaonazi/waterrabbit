        ...
        // Flip card toggle & Infinite Scroll Loop
        const flipContainer = document.querySelector('.flip-cards-container');
        if (flipContainer) {
            // Event delegation for flipping cards (works on cloned cards too)
            ...
            // Clone original cards to create a seamless scroll buffer
            const originalCards = Array.from(flipContainer.querySelectorAll('.flip-card'));
            
            // Clone them twice to ensure we have enough track to scroll infinitely
            for (let i = 0; i < 2; i++) {
                originalCards.forEach(card => {
                    const clone = card.cloneNode(true);
                    clone.classList.remove('fade-up'); // Prevent animation re-triggering
            ...
            flipContainer.addEventListener('scroll', () => {
                ...
                // Dynamically calculate the exact width of one full set of original cards
                const cardWidth = originalCards[0].offsetWidth;
                const gap = parseFloat(window.getComputedStyle(flipContainer).gap) || 0;
                const setWidth = (cardWidth + gap) * originalCards.length;

                // When near the very end, silently jump back by exactly one set width
                if (scrollLeft >= maxScrollLeft - 5) {
                    flipContainer.scrollLeft = scrollLeft - setWidth;
                } 
                // When at the absolute beginning, silently jump forward by one set width
                else if (scrollLeft <= 0) {
                    flipContainer.scrollLeft = scrollLeft + setWidth;
                }
            });
            
            // Set initial scroll position so the user can immediately scroll left if they want to
            setTimeout(() => {
                const cardWidth = originalCards[0].offsetWidth;
                const gap = parseFloat(window.getComputedStyle(flipContainer).gap) || 0;
